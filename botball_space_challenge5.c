...
int miniservo = 3;
int cyloarm_servo = 2;
int cyloarm_startpos = 1200;
int cyloarm_upamount = 800;
int cyloarm_upperamount = 500;
int astroarm_servo = 1; 
int astroclaw_servo = 0; 
int astroarm_startpos = 550;
int astroarm_downamount = 1156;
int astroarm_downamountagain = 850;
int astroarm_upamount = 600;
...
int main()
{
    ...
    crane_down_again();
    crane_up();
    //move to pool noodle place
    controlled_turn_right(1050);
    while(analog(lbumper) != 0)
    {
        forward();
    }
    ...
    enable_servos();
    set_servo_position(miniservo, miniservo_closed);
    return 0;
}
...
{
    enable_servos();
    while(cyloarm_startpos > cyloarm_upperamount)
    {
        set_servo_position(cyloarm_servo, cyloarm_startpos);
        msleep(50);
        cyloarm_startpos = cyloarm_startpos - 25;
...
{
    enable_servos();
    while(astroarm_startpos < astroarm_downamountagain)
    {
        set_servo_position(astroarm_servo, astroarm_startpos);
        msleep(50);
        astroarm_startpos = astroarm_startpos + 25;
